...
# VANIA COMMENT: wi_01, st_02 need reverse scoring if higher scores correspond to higher wealth inequality and higher social trust, respectively. Item st_01 currently also needs proofreading (it is currently "most people can be trusted or that you need to be very careful in dealing with people") and might need reverse scoring as well depending on how it is proofread

#get reversed cols

sswist_colsrev <- c("wi_01", "st_02")

#get non reversed cols

sswist_cols <- colnames(
  survey_4_coded %>% 
    select(starts_with(c("wi_", "st_", "ss_"))) %>%
             select(-ends_with(sswist_colsrev)))

#coding normal items
survey_4_coded <- funCode(survey_4_coded, sswist_cols, "agreement", FALSE)

#coding reversed items
survey_4_coded <- funCode(survey_4_coded, sswist_colsrev, "agreement", TRUE)
...
#    soc_network_size = sns_num,
    ...
    digust_animal = rowSums(across(paste0("ds_", sprintf("%02d",c(6, 7, 8, 9, 13, 19, 20, 21))))),

    #Disgust Sensitivity (contamination)
    digust_contamin = rowSums(across(paste0("ds_", sprintf("%02d",c(10, 11, 22, 24, 25))))))
...
survey_4_vars <- survey_4_scored %>%
  select(1, 2, 3, 4, 145:198, -apx1, -apx2)
