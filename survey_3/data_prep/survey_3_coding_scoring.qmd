...
# run the coding script - in the first instance on survey3, thereafter survey_3_coded
...
survey_3_scored <- survey_3_coded %>%
  mutate(
    ...
    #CRED (group leader)
    cred_group_ldr = rowMeans(across(c("cred_10", "cred_11", "cred_19", "cred_20"))), 
    
    #CRED (group member)
    cred_group_mbr = rowMeans(across(c("cred_12", "cred_13", "cred_14", "cred_15", "cred_16", "cred_21", "cred_22", "cred_23", "cred_24", "cred_25"))),
    
    #CRED (group)
    
    cred_group = rowMeans(across(c("cred_group_mbr", "cred_group_ldr"))),
    
    #CRED Exposure
    
    cred = rowMeans(across(c("cred_group", "cred_caregiver"))),
    ...
    crud_group_mbr = crud_02,
    
    #CRUD (group leader)
    
    crud_group_ldr = crud_03, 
    
    #CRUD (group)
    
    crud_group = rowMeans(across(c("crud_group_mbr", "crud_group_ldr"))),
    
    #CRUD
    
    crud = rowMeans(across(c("crud_group", "crud_caregiver"))),
...
survey_3_vars <- survey_3_scored %>%
  select(2, 3, 4, 5, 100:157, -apx1, -apx2) #VANIA COMMENT: fake data for survey 3 has an extra column in the beginning. Order of first columns goes X, ID, sex, age, country, while surveys 1 and 2 have not had X. If column X was added erroneously to the fake data and will not actually exist in the real surveys, then selected columns will be select (1, 2, 3, 4, 100:157). 
